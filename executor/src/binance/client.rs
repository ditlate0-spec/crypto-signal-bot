...
impl BinanceClient {
    ...
    pub async fn create_listen_key(&self) -> Result<String> {
        let resp: ListenKeyResponse = self.signed_post("/fapi/v1/listenKey", vec![]).await?;
        Ok(resp.listen_key)
    }

    pub async fn keepalive_listen_key(&self, listen_key: &str) -> Result<()> {
        let _: serde_json::Value = self.signed_put(
            "/fapi/v1/listenKey",
            vec![("listenKey".into(), listen_key.into())],
        ).await?;
        Ok(())
    }

    pub async fn close_listen_key(&self, listen_key: &str) -> Result<()> {
        let _: serde_json::Value = self.signed_delete(
            "/fapi/v1/listenKey",
            vec![("listenKey".into(), listen_key.into())],
        ).await?;
        Ok(())
    ...
    pub async fn get_price(&self, symbol: &str) -> Result<f64> {
        let r: PriceResponse = self.public_get("/fapi/v1/ticker/price", &[("symbol", symbol)]).await?;
        r.price.parse().map_err(|e: std::num::ParseFloatError| ExecutorError::Parse(e.to_string()))
    }

    pub async fn get_balance_usdt(&self) -> Result<f64> {
        let entries: Vec<BalanceEntry> = self.signed_get("/fapi/v2/balance", vec![]).await?;
        entries.into_iter().find(|b| b.asset == "USDT")
    ...
    pub async fn get_symbol_rules(&self, symbol: &str) -> Result<SymbolRules> {
        ...
        let s = info.symbols.into_iter().find(|s| s.symbol == symbol)
        ...
        for f in s.filters {
            match f {
                Filter::LotSize { step_size } => { rules.step_size = step_size.parse().unwrap_or(rules.step_size); }
                Filter::PriceFilter { tick_size } => { rules.tick_size = tick_size.parse().unwrap_or(rules.tick_size); }
                Filter::MinNotional { notional } => { rules.min_notional = notional.parse().unwrap_or(rules.min_notional); }
                Filter::Other => {}
            }
        }
        Ok(rules)
    ...
    pub async fn set_leverage(&self, symbol: &str, leverage: u32) -> Result<LeverageResponse> {
        self.signed_post("/fapi/v1/leverage", vec![
    ...
    pub async fn set_margin_type(&self, symbol: &str, margin_type: &str) -> Result<()> {
        let result: std::result::Result<MarginTypeResponse, ExecutorError> = self.signed_post(
            "/fapi/v1/marginType",
            vec![("symbol".into(), symbol.into()), ("marginType".into(), margin_type.into())],
        ).await;
        match result {
            Ok(_) => Ok(()),
    ...
    pub async fn open_short(&self, symbol: &str, quantity: f64) -> Result<OrderResponse> {
        self.signed_post("/fapi/v1/order", vec![
            ("symbol".into(), symbol.into()),
            ("side".into(), "SELL".into()),
    ...
    pub async fn get_order(&self, symbol: &str, order_id: i64) -> Result<OrderResponse> {
        self.signed_get("/fapi/v1/order", vec![
            ("symbol".into(), symbol.into()),
            ("orderId".into(), order_id.to_string()),
        ]).await
    }

    pub async fn wait_for_fill(&self, symbol: &str, order_id: i64, timeout_ms: u64) -> Result<OrderResponse> {
        ...
        loop {
            ...
            match order.status.as_str() {
                ...
                "CANCELED" | "REJECTED" | "EXPIRED" => {
                    return Err(ExecutorError::Binance { code: 0, msg: format!("order {} ended with status {}", order_id, order.status) });
                }
                _ => {}
            }
            if start.elapsed() >= timeout {
                return Err(ExecutorError::Binance { code: 0, msg: format!("order {} not filled within {}ms (status: {})", order_id, timeout_ms, order.status) });
            }
            tokio::time::sleep(std::time::Duration::from_millis(150)).await;
        }
    }

    pub async fn set_stop_loss(&self, symbol: &str, trigger_price: f64) -> Result<AlgoOrderResponse> {
        ...
        match result {
            ...
            Err(ExecutorError::Binance { code: -4120, .. }) => {
                tracing::warn!("algoOrder not supported, fallback to /fapi/v1/order");
                let _: OrderResponse = self.signed_post("/fapi/v1/order", vec![
                    ("symbol".into(), symbol.into()),
                    ("side".into(), "BUY".into()),
                    ("type".into(), "STOP_MARKET".into()),
                ...
                Ok(AlgoOrderResponse {
                    algo_id: 0, client_algo_id: String::new(),
                    algo_type: "CONDITIONAL".into(), order_type: "STOP_MARKET".into(),
                    symbol: symbol.into(), side: "BUY".into(), algo_status: "NEW".into(),
                    trigger_price: format!("{:.2}", trigger_price),
                    close_position: true, working_type: "MARK_PRICE".into(),
                })
            }
            Err(e) => Err(e),
        }
    ...
    pub async fn set_trailing_stop(&self, symbol: &str, quantity: f64, activation_price: f64, callback_rate: f64) -> Result<AlgoOrderResponse> {
        let result: std::result::Result<AlgoOrderResponse, ExecutorError> = self.signed_post(
            "/fapi/v1/algoOrder",
            vec![
                ("algoType".into(), "CONDITIONAL".into()),
        ...
        match result {
            ...
            Err(ExecutorError::Binance { code: -4120, .. }) => {
                ...
                Ok(AlgoOrderResponse {
                    algo_id: 0, client_algo_id: String::new(),
                    algo_type: "CONDITIONAL".into(), order_type: "TRAILING_STOP_MARKET".into(),
                    symbol: symbol.into(), side: "BUY".into(), algo_status: "NEW".into(),
                    trigger_price: format!("{:.2}", activation_price),
                    close_position: false, working_type: "MARK_PRICE".into(),
                })
            }
            Err(e) => Err(e),
        }
    ...
    pub async fn get_position(&self, symbol: &str) -> Result<Option<Position>> {
        ...
        for p in positions {
            if p.symbol == symbol {
                ...
                if amount.abs() < f64::EPSILON { return Ok(None); }
