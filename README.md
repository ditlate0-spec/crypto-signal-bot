...
Система состоит из **четырёх слоёв**, каждый решает свою задачу:
┌─────────────────────────────────────────────────────────────┐
...
▼
┌─────────────────────────────────────────────────────────────┐
│ 4. Фильтр по индексу страха и жадности │
│ Если индекс > 75 («Экстремальная жадность») — │
│ торговля блокируется. На пике эйфории рынок │
│ чаще разворачивается вверх, что опасно для шорта. │
└─────────────────────────────────────────────────────────────┘
```
...
|--------|--------------|
| Заголовок | «...через модель машинного обучения **и индекс страха и жадности**» |
| Схема | Четвёртый слой «Фильтр по индексу страха и жадности» (блок 4) |
| Результаты | Абзац про блокировку при Fear & Greed > 75 |
| Стек | Внешние API: добавлен **Fear & Greed Index (alternative.me)** |
| Что реализовано | Пункт «Фильтр по индексу страха и жадности» |
| Структура | `TradingBotService.php` — уточнён порядок: KF → вердикт → ML → Fear&Greed → сделка |
| Ограничения | Пункт про то, что фильтр >75 сокращает число сделок в бычьи периоды |
